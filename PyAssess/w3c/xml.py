...
from PyAssess.unicode.ucd import MakeCharClass, CharInClass
from xmlnamespaces import XMLNamespace

class XMLError(Exception): pass
class UndefinedNamespace(XMLError): pass

class XMLFile:
	def __init__(self,f,allowUnicode=0):
		self.f=f
		...
		self.indent=0
		self.indentString="\n"
		self.namespaces={XMLNamespace:'xml'}
	...
	def AppendStartTag(self,e,indentChildren=0):
		if isinstance(e,Element):
			nsChanges=self.UpdateNamespaces(e)
			self.f.write(self.indentString+e.StartTag())
			if indentChildren:
				self.etags.append((
					self.indentString+e.EndTag(),
					self.indent,
					self.indentString,
					nsChanges))
				self.indent+=1
	...
	def AppendEmptyTag(self,e):
		if isinstance(e,Element):
			nsChanges=self.UpdateNamespaces(e)
			self.f.write(self.indentString+e.EmptyTag())
			self.ApplyNamespaceChanges(nsChanges)
		else:
			raise TypeError
	
	def AppendComment(self,comment):
		if type(comment) in StringTypes:
			comment=Comment(comment)
		if isinstance(comment,Comment):
			self.f.write(self.indentString+str(comment))
		else:
			raise TypeError
						
	def UpdateNamespaces(self,e):
		undoDefinitions={}
		for attribute in e.attrs:
			if attribute.name[:6]=="xmlns:":
				prefix=attribute.name[6:]
				undoDefinitions[attribute.value]=self.namespaces.get(attribute.value)
				self.namespaces[attribute.value]=prefix
		if e.ns:
			prefix=self.namespaces.get(e.ns,None)
			if prefix is None:
				# add a suitable prefix definition
				undoDefinitions[e.ns]=None
				self.namespaces[e.ns]=''
				prefix=''
				# prepend the namespace defining attribute
				e.attrs=[Attribute('','xmlns',e.ns)]+e.attrs
			elif prefix:
				# apply the prefix to the element name
				e.name=prefix+':'+e.name
		for attribute in e.attrs:
			if attribute.ns:
				prefix=self.namespaces.get(attribute.ns)
				if prefix is None:
					raise UndefinedNamespace
				elif prefix:
					attribute.name=prefix+':'+attribute.name
		return undoDefinitions
	
	def AppendCDATA(self,cdata):
		cdata=join(split(cdata,']]>'),']]>]]&gt;<![CDATA[')
		self.f.write('<![CDATA['+cdata+']]>')
		
	def AppendData(self,pcdata):
		self.f.write(EscapeCharData(pcdata,self.allowUnicode).encode('utf-8'))

	def AppendEndTag(self):
		etag,self.indent,self.indentString,nsChanges=self.etags.pop()
		self.f.write(etag)
		self.ApplyNamespaceChanges(nsChanges)
		
	def ApplyNamespaceChanges(self,nsChanges):
		for ns in nsChanges.keys():
			if nsChanges[ns] is None:
				del self.namespaces[ns]
			else:
				self.namespaces[ns]=nsChanges[ns]
...
def IsBaseChar(c):
	return c and CharInClass(c,BaseCharClass)
...
def IsCombiningChar(c):
	return c and CharInClass(c,CombiningCharClass)

def IsDigit(c):
	return c and CharInClass(c,DigitClass)

def IsExtender(c):
	return c and CharInClass(c,ExtenderClass)
...
class Element:
	def __init__(self,ns,name,attrs=[],data=None):
		self.ns=ns
		self.name=name
	...
	def SetAttribute(self,ns,name,value):
		self.attrs.append(Attribute(ns,name,value))
	
	def StartTag(self):
		tag=['<',str(self.name)]
		for attribute in self.attrs:
			tag.append(' '+str(attribute))
		tag.append('>')
		return join(tag,'')
		
	def UnicodeStartTag(self):
		tag=[u'<',unicode(self.name)]
		for attribute in self.attrs:
			tag.append(u' '+unicode(attribute))
		tag.append(u'>')
		return join(tag,'')
	...
	def UnicodeEndTag(self):
		tag=[u'</',unicode(self.name),u'>']
		return join(tag,'')
	...
	def __str__(self):
		if self.data is None:
			...
			return join([self.StartTag(),EscapeCharData(self.data),self.EndTag()])
		
	def __unicode__(self):
		if self.data is None:
			return self.UnicodeEmptyTag()
		else:
			return join([self.UnicodeStartTag(),EscapeCharData(self.data,1),self.UnicodeEndTag()])


class StartTag(Element): pass

class EmptyTag(Element): pass
...
class Attribute:
	...
	def __str__(self):
		return join((self.name,'="',str(EscapeAttributeValue(self.value,0)),'"'),'')
	
	def __unicode__(self):
		return join((self.name,u'="',unicode(EscapeAttributeValue(self.value,1)),u'"'),'')	
...
def ParseXMLCharClass(src):
	"""Internal function for building a character classes from the format
	of table published in the XML specification."""
	strippedSrc=[]
	charClass=[]
	for c in src:
		if c in "abcdefABCDEF0123456789-|":
			strippedSrc.append(c)
	for rangeDef in split(join(strippedSrc,''),'|'):
		r=split(rangeDef,'-')
		if len(r)==1:
			c=unichr(int(r[0],16))
			charClass.append([c,c])
		elif len(r)==2:
			charClass.append([unichr(int(r[0],16)),unichr(int(r[1],16))])
	MakeCharClass(charClass)
	return charClass
			
	
BaseCharClass=ParseXMLCharClass("""[#x0041-#x005A] | [#x0061-#x007A] | [#x00C0-#x00D6] | [#x00D8-#x00F6]
...
IdeographicCharClass=ParseXMLCharClass("""[#x4E00-#x9FA5] | #x3007 | [#x3021-#x3029]""")

CombiningCharClass=ParseXMLCharClass("""[#x0300-#x0345] | [#x0360-#x0361] | [#x0483-#x0486]
...
DigitClass=ParseXMLCharClass("""[#x0030-#x0039] | [#x0660-#x0669] | [#x06F0-#x06F9]
...
ExtenderClass=ParseXMLCharClass("""#x00B7 | #x02D0 | #x02D1 | #x0387 | #x0640 | #x0E46 | #x0EC6
| #x3005 | [#x3031-#x3035] | [#x309D-#x309E] | [#x30FC-#x30FE]""")

LetterClass=MakeCharClass(BaseCharClass+IdeographicCharClass)

NameCharClass=MakeCharClass(LetterClass+DigitClass+[['.','.'],['-','-'],['_','_'],[':',':']]+CombiningCharClass+ExtenderClass)
