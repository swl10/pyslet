...
class SessionTest(unittest.TestCase):
	def setUp(self):
		self.item=AssessmentItem("test","Session Test Item",False,False)
	...
	def testEndAttempt(self):
		...
		session.EndAttempt({})
		# No modal feedback in the item so it returns to interacting state
		self.failUnless(session.userState==AssessmentItemState.Interacting,"end attempt state test")
		self.failUnless(session.GetResponseValue('RESPONSE')==
