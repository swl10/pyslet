...
class URIReferenceTests(unittest.TestCase):
	...
	def testResolution(self):
		"""Check resolve method"""
		"""Note that a reference consisting solely of a fragment means to use the current document, but for
		resolution purposes we require the original URL to be returned.  It is not clear exactly what is
		meant from the RFC but it leads us to conclude that there may be a difference between giving
		the current URL or leaving it implicit.  Perhaps the intention is that an omitted URI never results
		in the resource being reloaded (meaningful for http schems) whereas giving the URI might cause the
		cache to be checked and a reload issued if necessary.  I don't know if people really treat it this
		way though."""
		baseURI=AbsoluteURI("http://a/b/c/d;p?q")
		tests=[
			('g:h','g:h'),
			('g','http://a/b/c/g'),
			('./g','http://a/b/c/g'),
			...
			('?y','http://a/b/c/?y'),
			('g?y','http://a/b/c/g?y'),
#			('#s','(current document)#s'), - see note above...
		...
		for t in tests:
			testStr=repr(URIReference(t[0]).Resolve(baseURI))
			self.failUnless(testStr=="URIReference('"+t[1]+"')","%s + %s gave %s expected %s"%(str(baseURI),t[0],testStr,t[1]))
...
class ParserTests(unittest.TestCase):
	...
	def testParseMethods(self):
		tests=(
			...
			("ParseRelativeURI","?c",
				RelativeURI(None,URIQuery('c'))),
			("ParseRelativeURI","?",
				RelativeURI(None,URIQuery(''))),
			...
			("ParseNetPath","///",URINetPath(None,URIAbsPath([URISegment('')]))),
