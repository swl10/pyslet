...
class GenericParserTests(unittest.TestCase):
    ...
    def testCaseBasic(self):
        ...
        for i in xrange(0, 256):
            c = chr(i)
            self.assertTrue(IsHEX(c) == (c in HEX), "IsHEX(chr(%i))" % i)
    ...
    def testCaseTokenList(self):
        ...
        self.assertTrue(
            p.ParseTokenList() == ["a", "b", "c", "d", "efg"], "Bad token list")
        self.assertFalse(p.cWord, "Expected no more words")
        self.assertFalse(p.IsToken(), "Expected no token")
    ...
    def testCaseParameter(self):
        ...
        self.assertTrue(parameters == {'x': ['X', '1'], 'y': ['y', '2'], 'zoo': [
                        'Zoo', ';A="Three"']}, "Paremters: %s" % repr(parameters))
        try:
            parameters = {}
        ...
        self.assertTrue(
            parameters == {'x': ['X', '1']}, "Paremters: %s" % repr(parameters))
        parameters = {}
        p.ParseParameters(parameters)
        self.assertTrue(parameters == {'q': ['q', '2'], 'zoo': [
                        'Zoo', ';A="Three"']}, "Paremters: %s" % repr(parameters))
        parameters = {}
        p = WordParser(' ;X=1 ;y=2;Zoo=";A=\\"Three\\""')
...
class ProtocolParameterTests(unittest.TestCase):
    ...
    def testCaseFullDate(self):
        ...
        timestamp850 = FullDate.FromHTTPString(
            "Sunday, 06-Nov-94 08:49:37 GMT")
        # ANSI C's asctime() format
        timestampC = FullDate.FromHTTPString("Sun Nov  6 08:49:37 1994")
        self.assertTrue(
            timestamp822 == timestamp850, "RFC 850 timestamp parser")
        ...
        try:
            # Weekday mismatch
            timestamp822 = FullDate.FromHTTPString(
        ...
        timestamp822 = FullDate.FromHTTPString("Sun, 06 Nov 1994 08:49:37 GMT")
        self.assertTrue(
            str(timestamp822) == "Sun, 06 Nov 1994 08:49:37 GMT", "All-in-one parser")
    ...
    def testCaseTransferEncoding(self):
        ...
        self.assertTrue(te.parameters == {'x': ['x', '1'], 'y': [
                        'y', '2']}, "Extension parameters: %s" % repr(te.parameters))
        ...
        self.assertTrue(te.parameters == {
                        'a': ['a', '4']}, "Single extension parameters: %s" % repr(te.parameters))
        try:
    ...
    def testCaseMediaType(self):
        ...
        self.assertTrue(mtype.parameters == {'charset': [
                        'Charset', 'en-US'], 'x': ['x', '1']}, "Media type parameters: %s" % repr(mtype.parameters))
        self.assertTrue(
    ...
    def testCaseProductToken(self):
        ...
        self.assertTrue(ProductToken.Explode("2.17b3") == (
            (2,), (17, "b", 3)), "Complex explode: %s" % repr(ProductToken.Explode("2.17b3")))
        ...
        self.assertTrue(
            ProductToken("Apache", "0.8.200") > ProductToken("Apache", "0.8.30"))
        self.assertTrue(
            ProductToken("Apache", "0.8.4") == ProductToken("Apache", "0.8.04"))
        self.assertTrue(
            ProductToken("Apache", "0.8.4") > ProductToken("Apache", "0.8b1.4"))
        self.assertTrue(
            ProductToken("Apache", "1b4.8.4") > ProductToken("Apache", "0.8.4"))
    ...
    def testCaseLanguageTag(self):
        ...
        self.assertTrue(
            lang == LanguageTag.FromString('en-us'), "case insensitive comparison")
        ...
        self.assertTrue(LanguageTag.FromString(
            "en-US") > LanguageTag.FromString("en-gb"), "case insensitive order")
        # test for hash
        self.assertTrue(hash(LanguageTag.FromString(
            "en-us")) == hash(LanguageTag.FromString("en-US")), "case insensitive hash")
    ...
    def testCaseEntityTag(self):
        ...
        self.assertTrue(
            eTag.tag == 'h"ello', "Failed to unpick quoted pair from ETag value; found %s" % repr(eTag.tag))
...
class HeaderTests(unittest.TestCase):
    ...
    def testCaseMediaRange(self):
        ...
        self.assertTrue(len(
            mr.parameters) == 1, "q-value terminates parameters: %s" % repr(mr.parameters))
        # check comparisons
        self.assertTrue(MediaRange.FromString(
            "image/*") < MediaRange.FromString("*/*"), "Specific over general */*")
        self.assertTrue(MediaRange.FromString(
            "image/png") < MediaRange.FromString("image/*"), "Specific over general image/*")
        self.assertTrue(MediaRange.FromString("text/plain;charset=utf-8") <
                        MediaRange.FromString("text/plain"), "Parameter over no-parameter")
    ...
    def testCaseAcceptList(self):
        ...
        self.assertTrue(
            str(al[0]) == "audio/basic", "don't add 1 for defaults: %s" % str(al[0]))
        ...
        self.assertTrue(
            str(al) == "text/html, text/plain; q=0.5, text/x-c, text/x-dvi; q=0.8", str(al))
        al = AcceptList.FromString("text/*, text/html, text/html;level=1, */*")
        self.assertTrue(
            str(al) == "text/html; level=1, text/html, text/*, */*", str(al))
        mediaTypeList = [MediaType.FromString("text/html;level=1"), MediaType.FromString(
            "text/html"), MediaType.FromString("text/html;level=2"), MediaType.FromString("text/xhtml")]
        ...
        # first in list
        self.assertTrue(str(bestType) == "text/html; level=1", str(bestType))
        al = AcceptList.FromString(
        ...
        #	text/xhtml			: q=1.0		partial match on text/*
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(str(bestType) == "text/xhtml",
                        "Specific match with confusing q value: %s" % str(bestType))
        del mediaTypeList[3]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(
            str(bestType) == "text/html", "beats level 2 only on order in list")
        del mediaTypeList[1]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(str(bestType) == "text/html; level=2",
        ...
        mediaTypeList = [MediaType.FromString("text/html;level=1"), MediaType.FromString("text/html"),
                         MediaType.FromString(
                             "text/plain"), MediaType.FromString("image/jpeg"), MediaType.FromString("text/html;level=2"),
        ...
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(
            str(bestType) == "text/html; level=1", "Only exact match with q=1")
        del mediaTypeList[0]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(
            str(bestType) == "text/html", "beats level=3 on order in list")
        del mediaTypeList[0]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(
            str(bestType) == "text/html; level=3", "matches text/html")
        del mediaTypeList[-1]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(
            str(bestType) == "image/jpeg", "matches */*, returned %s" % str(str(bestType)))
        del mediaTypeList[1]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(
            str(bestType) == "text/html; level=2", "exact match with q=0.4")
        del mediaTypeList[1]
        bestType = al.SelectType(mediaTypeList)
        self.assertTrue(str(bestType) == "text/plain", "matches text/*")
        al = AcceptList.FromString(
            "text/*, text/html, text/html;level=1, image/*; q=0, image/png; q=0.05")
        ...
        self.assertTrue(
            str(bestType) == "image/png", "Best partial match: %s" % str(bestType))

    def testCaseAcceptTokenList(self):
        rqToken = AcceptToken()
        self.assertTrue(
            rqToken.token == "*" and rqToken.q == 1.0, "AcceptToken constructor")
        self.assertTrue(str(rqToken) == "*", "AcceptToken Format default")
        rqToken = AcceptToken("gzip", 0.5)
        self.assertTrue(str(rqToken) == "gzip;q=0.5",
                        "AcceptToken custom constructor Format default: %s" % str(rqToken))
        rqTokens = AcceptTokenList.FromString(
            " gzip;q=1.0, identity; q=0.5, *;q=0")
        self.assertTrue(rqTokens[0].token == 'gzip' and rqTokens[0].q == 1.0,
                        "Parse accept encodings: gzip;q=1.0")
        self.assertTrue(str(
            rqTokens[0]) == "gzip", "Format accept encodings: found %s" % str(rqTokens[0]))
        self.assertTrue(rqTokens[1].token == 'identity' and rqTokens[1].q == 0.5,
                        "Accept encodings identity;q=0.5")
        self.assertTrue(
            str(rqTokens[1]) == "identity;q=0.5", "Format accept encodings: identity;q=0.5")
        self.assertTrue(rqTokens[2].token == '*' and rqTokens[2].q == 0,
                        "Accept encodings *;q=0")
        self.assertTrue(str(
            rqTokens[2]) == "*;q=0", "Format accept encodings: found %s" % str(rqTokens[2]))
    ...
    def testCaseAcceptCharsetList(self):
        ...
        rqTokens = AcceptCharsetList.FromString(
            " utf-8;q=1.0, symbol; q=0.5, *;q=0.5")
        self.assertTrue(
            rqTokens.SelectToken(["iso-8859-1"]) is not None, "match *")
        # so if * is excluded then it will be excluded
        rqTokens = AcceptCharsetList.FromString(
            " utf-8;q=1.0, symbol; q=0.5, *;q=0")
        self.assertTrue(
            rqTokens.SelectToken(["iso-8859-1"]) is None, "match * q=0")
        # and if * is not present it gets q value 1
        rqTokens = AcceptCharsetList.FromString(" utf-8;q=1.0, symbol; q=0.5")
        self.assertTrue(rqTokens.SelectToken(
            ["symbol", "iso-8859-1"]) == "iso-8859-1", "default q=1 for latin-1")

    def testCaseAcceptEncodingList(self):
        rqTokens = AcceptEncodingList.FromString("compress, gzip")
        self.assertTrue(
            rqTokens.SelectToken(["gzip"]) is not None, "match token")
        rqTokens = AcceptEncodingList.FromString("compress, gzip;q=0")
        self.assertTrue(
            rqTokens.SelectToken(["gzip"]) is None, "match token unless q=0")
        rqTokens = AcceptEncodingList.FromString("compress, *, gzip;q=0")
        self.assertTrue(
            rqTokens.SelectToken(["gzip"]) is None, "match token unless q=0; unmatched *")
        ...
        self.assertTrue(
            rqTokens.SelectToken(["compress", "gzip"]) == "gzip", "match highest q")
        rqTokens = AcceptEncodingList.FromString(
        ...
        rqTokens = AcceptEncodingList.FromString(
            "compress; q=0.5, gzip;q=0.75")
        self.assertTrue(
            rqTokens.SelectToken(["identity"]) is not None, "identity acceptable")
        rqTokens = AcceptEncodingList.FromString(
            "compress; q=0.5, gzip;q=0.75, identity;q=0")
        self.assertTrue(
            rqTokens.SelectToken(["identity"]) is None, "identity unacceptable")
        rqTokens = AcceptEncodingList.FromString(
        ...
        rqTokens = AcceptEncodingList.FromString("")
        self.assertTrue(
            rqTokens.SelectToken(["identity"]) is not None, "identity acceptable (empty)")
        self.assertTrue(
            rqTokens.SelectToken(["gzip"]) is None, "gzip unacceptable (empty)")

    def testCaseAcceptLanguageList(self):
        rqTokens = AcceptLanguageList.FromString(" da, en-gb;q=0.8, en;q=0.7 ")
        self.assertTrue(
            rqTokens.SelectToken(["en-US"]) == "en-US", "match prefix")
        self.assertTrue(
            type(rqTokens.SelectToken(["en-US"])) in StringTypes, "SelectToken return type")
    ...
    def testCaseAcceptRanges(self):
        ...
        self.assertTrue(
            str(ar2) == "Bits, Bytes", "str preserves order and case but not spae")
        try:
    ...
    def testCaseAllow(self):
        ...
        self.assertTrue(
            str(allow2) == "PUT, GET, HEAD", "str preserves order but not case or space")
    ...
    def testCaseCacheControl(self):
        ...
        self.assertTrue(
            str(cc) == "no-transform, ext=token, ext2=\"token=4\"", "Token and Quoted string")
    ...
    def testCaseContentRange(self):
        ...
        self.assertTrue(
            cr1.firstByte == 0 and cr1.lastByte == 499 and cr1.totalLength == 1234)
...
class HTTP2616Tests(unittest.TestCase):
    ...
    def testCaseManager(self):
        ...
        self.assertTrue(str(response1.protocolVersion) == "HTTP/1.1",
                        "Protocol in response1: %s" % response1.protocolVersion)
        self.assertTrue(
            response1.status == 200, "Status in response1: %i" % response1.status)
        ...
        self.assertTrue(
            response2.status == 200, "Status in response2: %i" % response2.status)
    ...
    def testCaseContinue(self):
        ...
        self.assertTrue(
            response1.status == 200, "Status in response1: %i" % response1.status)
        self.assertTrue(
            response1.reason == "OK", "Reason in response1: %s" % response1.reason)
        self.assertTrue(
        ...
        self.assertTrue(
            response2.status == 200, "Status in response2: %i" % response2.status)
        self.assertTrue(
            response2.reason == "OK", "Reason in response2: %s" % response2.reason)
        self.assertTrue(
            request2.resBody == "", "Data in response2: %s" % request2.resBody)
    ...
    def testCaseStreamedPut(self):
        ...
        rm.httpUserAgent = None
        request = HTTPRequest(
            "http://www.domain1.com/file2", "PUT", StringIO.StringIO("123456\r\n\r\n"))
        ...
        self.assertTrue(
            response.reason == "OK", "Reason in response: %s" % response.reason)
        self.assertTrue(
            request.resBody == "", "Data in response: %s" % request.resBody)
        request = HTTPRequest(
            "http://www.domain1.com/file", "PUT", StringIO.StringIO("123456\r\n\r\n"))
    ...
    def testCaseStreamedGet(self):
        ...
        self.assertTrue(
            response.status == 200, "Status in response: %i" % response.status)
        self.assertTrue(
            buff.getvalue() == TEST_STRING, "Data in response: %s" % request.resBody)
        self.assertTrue(
            request.resBody == "", "Data in streamed response: %s" % request.resBody)
...
class ThreadedTests(unittest.TestCase):
    ...
    def testCaseMultiGET(self):
        ...
        for i in xrange(10):
            threads.append(
                threading.Thread(target=Domain3ThreadOneShot, args=(rm,)))
        for i in xrange(10):
            threads.append(
                threading.Thread(target=Domain4ThreadOneShot, args=(rm,)))
        for t in threads:
            t.start()
        while threads:
            t = threads.pop()
