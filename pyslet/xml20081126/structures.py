...
class XMLForbiddenEntityReference(XMLError):
    pass
...
class XMLUnimplementedError(XMLError):
    pass


class XMLUnexpectedError(XMLError):
    pass
...
DiscouragedCharClass = CharClass(
    (u'\x7f', u'\x84'), (u'\x86', u'\x9f'), (u'\ufdd0', u'\ufdef'))
        ...
        try:
            if factoryName:
                ...
                if isinstance(factory, MethodType):
                    ...
                    raise TypeError(
                        factoryName, repr(factory), repr(childClass))
...
class XMLSequenceList(XMLContentParticle):
    ...
    def BuildParticleMaps(self, exitParticles):
        ...
        for i in xrange(len(self.children)):
            ...
            # but if child+1 is optional then we have to include child+2, and
            # so on...
            childExits = {}
            j = i + 1
            while True:
...
class XMLEntity(object):
    ...
    MagicTable = {
        ...
        '\x00\x00\xfe\xff': ('utf_32_be', 4, True),
        # UCS-4, little-endian machine (4321 order)
        '\xff\xfe\x00\x00': ('utf_32_le', 4, True),
        ...
        '\x3c\x00\x00\x00': ('utf_32_le', 0, False),
        # UCS-4 or other encoding with an unusual 32-bit code unit
        '\x00\x00\x3c\x00': ('utf_32_le', 0, False),
        # UCS-4 or other encoding with an unusual 32-bit code unit
        '\x00\x3c\x00\x00': ('utf_32_le', 0, False),
        # UTF-16BE or big-endian ISO-10646-UCS-2 or other encoding with a
        # 16-bit code unit
        '\x00\x3c\x00\x3f': ('utf_16_be', 0, False),
    ...
    def ChangeEncoding(self, encoding):
        ...
        if self.dataSource:
            ...
            # that will force BOM detection and we may not have a BOM to
            # detect.
            if not keepExisting:
                self.encoding = encoding
                # Need to rewind and re-read the current buffer
                self.charSource.seek(self.charSeek)
                self.charSource = codecs.getreader(
                    self.encoding)(self.dataSource)
