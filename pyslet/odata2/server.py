...
class Server(app.Server):
	...
	def __call__(self,environ, start_response):
		...
		try:
			...
			return self.ODataError(ODataURI('error'),environ,start_response,"Bad Request","Method not allowed: %s"%str(e),400)			
		except ValueError,e:
	...
	def GetResource(self,odataURI):
			...
			if isinstance(resource,edmx.Edmx):
				...
				if isinstance(resource,edm.DeferredValue):
					if resource.isCollection:
						...
						if keyPredicate:
							raise BadURISegment("%s (keyPredicate not allowed for a navigation property that identifies a single entity)"%name)
				...
				raise MissingURISegment("%s, property cannot be resolved as the parent entity does not exist"%name)
			else:
		...
		if isinstance(resource,edm.EntityCollection):
			...
			# there is a very specific use case here, use of <entity>/$links/<nav-et> where the link is NULL
			if not odataURI.pathOption==PathOption.links:
				# <entity>/<nav-et> where the link is NULL raises a 404
				# See the resolution: https://tools.oasis-open.org/issues/browse/ODATA-412
				raise MissingURISegment("no entity is linked by this navigation property")
	...
	def SetETag(self,entity,responseHeaders):
		...
		if etag is not None:
			...
			etag=s%http.QuoteString(string.join(map(ODataURI.FormatLiteral,etag),','))
		...
		*	*request* is an :py:class:`ODataURI` instance with a non-empty resourcePath."""
		...
		try:
			...
			if request.pathOption==PathOption.metadata:
				...
				# request.linksProperty is the name of the navigation property in the source entity
				# resource will be the target entity, a collection or None 
				...
				if method=="GET":
					...
					if resource is None:
						# can you POST to Orders(1)/$links/Customer ? - only if it is currently NULL (0..1)
						resource=parentEntity[request.linksProperty].OpenCollection()
					if isinstance(resource,edm.EntityCollection):
						with resource as collection:
						...
						raise BadURISegment("%s is already linked, use PUT instead of POST to update it"%request.linksProperty)
				elif method=="PUT":
					...
					if isinstance(resource,edm.EntityCollection):
						raise BadURISegment("%s: DELETE must specify a single link"%request.linksProperty)
					elif resource is None:
						raise MissingURISegment("%s, no entity is related"%request.linksProperty)						
					with parentEntity[request.linksProperty].OpenCollection() as collection:
				...
				if method=="GET":
					...
					if request.pathOption==PathOption.value:
						...
						resource.Update()
						# now we've updated the entity it is safe to calculate the ETag
						self.SetETag(resource,responseHeaders)
						return self.ReturnEmpty(start_response,responseHeaders)
				elif method=="DELETE":
					if request.pathOption==PathOption.value:
						raise BadURISegment("$value cannot be used with DELETE")
					resource.Delete()
					return self.ReturnEmpty(start_response,responseHeaders)
				else:
					raise InvalidMethod("%s not supported here"%method)
				...
				if method=="GET":
					if request.pathOption==PathOption.value:
						if resource:
							return self.ReturnDereferencedValue(parentEntity,resource,request,environ,start_response,responseHeaders)
						else:
							raise MissingURISegment("%s (NULL)"%resource.pDef.name)
					else:
						return self.ReturnValue(parentEntity,resource,request,environ,start_response,responseHeaders)
				elif method=="PUT":
					if request.pathOption==PathOption.value:
						if resource:
	...
	def ReadValue(self,value,environ):
		...
		if isinstance(input,Document):
			if isinstance(input.root,Property):
				...
				raise InvalidData("Unable to parse property from request body (found <%s>)"%doc.root.xmlname)
		else:
	...
	def ReturnValue(self,entity,value,request,environ,start_response,responseHeaders):
	...
	def ReadDereferencedValue(self,value,environ):
		...
		if value.mType is None:
			...
			if requestType:
				...
				acceptType=True
		if not acceptType:
			raise InvalidData("Unable to parse property value from request body (found <%s>)"%str(requestType))
	...
	def ReturnDereferencedValue(self,entity,value,request,environ,start_response,responseHeaders):
