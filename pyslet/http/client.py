...
from pyslet.http.messages import *
...
class Connection(object):
                ...
                if self.sendBuffer:
                    ...
                    if self.continueWaitStart:
                        if time.time() - self.continueWaitStart > self.continueWaitMax:
                            logging.warn(
                                "%s timeout while waiting for 100-Continue response; sending anyway")
    ...
    def Kill(self):
        ...
        with self.connectionLock:
            ...
            if not self.connectionClosed and self.socket:
                try:
                    logging.warn(
                        "Connection.Kill forcing socket shutdown for %s", self.host)
    ...
    def _StartRequest(self, request):
        ...
        if self.request.GetExpectContinue():
            ...
            self.requestMode = self.REQ_BODY_SENDING
        logging.debug(
            "%s: request mode=%s", self.host, self.MODE_STRINGS[self.requestMode])
        if self.response:
    ...
    def _RecvResponseData(self):
        ...
        if data:
            ...
            # related request
            logging.debug(
                "%s: closing connection after recv returned no data on ready to read socket", self.host)
            self.Close()
            return True
        ...
        while self.response is not None:
            ...
            if recvNeeds == CRLF:
                ...
                if nBytes is 0:
                    ...
                    logging.debug(
                        "Response waiting for %s bytes", str(nBytes - self.recvBufferSize))
                    ...
                    while gotBytes < nBytes:
                        ...
                        if gotBytes + len(data) < nBytes:
                            ...
                            bytes = string.join(
                                self.recvBuffer[0:buffPos + 1], '')
                            self.recvBuffer = self.recvBuffer[buffPos + 1:]
                            break
                        else:
                            # Tricky case, only some of this string is needed
                            bytes = string.join(
                                self.recvBuffer[0:buffPos] + [data[0:nBytes - gotBytes]], '')
                            self.recvBuffer = [
                                data[nBytes - gotBytes:]] + self.recvBuffer[buffPos + 1:]
                            break
        ...
        try:
            with self.connectionLock:
                if self.socket is not None:
                    ...
                    logging.info(
                        "Connected to %s with %s, %s, key length %i", self.host, *self.socket.cipher())
        except socket.error, err:
            raise HTTPException(
...
class HTTPRequestManager(object):
    ...
    def QueueRequest(self, request, timeout=None):
        ...
        threadTarget = (
            threadId, request.scheme, request.hostname, request.port)
        ...
        with self.managerLock:
            ...
            while True:
                ...
                # Step 5: wait for something to change
                    ...
                    if timeout == 0:
                        logging.warn(
                            "non-blocking call to QueueRequest failed to obtain an HTTP connection")
                        raise RequestManagerBusy
                    elif timeout is not None and now > start + timeout:
                        logging.warn(
                            "QueueRequest timed out while waiting for an HTTP connection")
                        raise RequestManagerBusy
                    logging.debug(
                        "QueueRequest forced to wait for an HTTP connection")
                    self.managerLock.wait(timeout)
                    logging.debug(
                        "QueueRequest resuming search for an HTTP connection")
    ...
    def FindCredentials(self, challenge):
        ...
        logging.debug("HTTPRequestManager searching for credentials in %s with challenge %s",
...
class HTTPMessage(object):
    ...
    def __init__(self):
        #: a lock used by :py:class:`HTTPClient`
        ...
        with self.lock:
            ...
            if fieldValue is None:
                ...
                if fieldNameKey in self.headers and appendMode:
                    fieldValue = self.headers[
                        fieldNameKey][1] + ", " + fieldValue
    ...
    def SetConnection(self, connectionTokens):
        ...
        if connectionTokens:
            self.SetHeader(
                "Connection", string.join(list(connectionTokens), ", "))
        else:
    ...
    def SetContentEncoding(self, contentCodings):
        ...
        *content-coding* tokens.  If the list is empty any existing
        ...
        if contentCodings:
            self.SetHeader(
                "Content-Encoding", string.join(list(contentCodings), ", "))
        else:
    ...
    def SetContentLanguage(self, langList):
        ...
        if langList:
            self.SetHeader(
                "Content-Language", string.join(map(str, langList), ", "))
        else:
    ...
    def CalculateTransferLength(self, body=None):
        ...
        if contentLength is not None:
            ...
            if body is not None:
                # If we know the body then raise errors if these don't match
                # our calculation
        ...
        if isinstance(self, HTTPRequest):
            if body is None:
                ...
                # We have a non empty body but there are no headers set, add in
                # the Content-Length
                self.SetContentLength(len(body))
                self.transferLength = len(body)
                return
            else:
                # If it is GET, HEAD or DELETE request with an empty body we
                # don't set headers or send an entity!
                return
        else:
            # If it is a response with an unknown transfer length: we'll read
            # forever!
            return
...
class HTTPRequest(HTTPMessage):
    ...
    def SetRequestURI(self, url):
        ...
        if not isinstance(self.requestURI, HTTPURL):
            raise HTTPException(
                "Scheme not supported: %s" % self.requestURI.scheme)
    ...
    def ReadRequestHeader(self):
        ...
        logging.info(
            "%s %s %s", self.method, self.url, str(self.protocolVersion))
    ...
    def ReadRequestBody(self):
        ...
        if self.reqBodyStream:
            ...
            if self.transferChunked:
                ...
                # This else handles an empty body too, not chunked and no
                # transferLength
                self.transferPos = len(self.reqBody)
                self.transferDone = True
                return self.reqBody
    ...
    def HandleResponseHeader(self):
        ...
        logging.debug(
            "Request: %s %s %s", self.method, self.url, str(self.protocolVersion))
        logging.debug(
            "Got Response: %i %s", self.response.status, self.response.reason)
    ...
    def ResponseFinished(self, err=None):
        ...
        if self.status is None:
            ...
            if self.response.status >= 100 and self.response.status <= 199:
                ...
                # The response was received before the connection finished with
                # us
                if self.status >= 300:
    ...
    def RecvLine(self, line):
        if self.mode == self.RESP_STATUS:
            ...
            if statusLine.is_integer():
                ...
                self.status = 0
            statusLine.parse_sp()
            self.reason = statusLine.parse_remainder()
            ...
            raise HTTPException(
                "RecvLine when in unknown mode: %i" % self.mode)
        if self.mode == self.RESP_DONE:
            self.Finished()
    ...
    def ReadHeader(self, h):
        if h == CRLF:
            if self.currHeader:
                ...
                self.SetHeader(
                    self.currHeader[0], self.currHeader[1][:-2], True)
                self.currHeader = None
            return False
        else:
            fold = OctetParser(h).parse_lws()
            if fold and self.currHeader:
                ...
                if len(ch) == 2:
                    ...
                    raise HTTP2616ProtocolError(
                        "Badly formed header line: %s" % h)
            return True
