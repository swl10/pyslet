			...
			for s in self.Schema:
				if name in s:
					...
					if container is self.defaultContainer:
						continue
					return resource
		raise KeyError("No entity set or service operation with name %s"%name)
...
def ValidateMetadataDocument(doc):
	warnings.warn("ValidateMetadataDocument is deprecated, use Validate method instead", DeprecationWarning, stacklevel=3)	
	return doc.Validate()
		...
		for container in self.root.FindChildrenDepthFirst(edm.EntityContainer):
			try:
				...
				if flag=="true":
					...
					raise InvalidMetadataDocument("IsDefaultEntityContainer: %s"%flag)
			except KeyError:
				pass
		if nDefaults!=1:
			raise InvalidMetadataDocument("IsDefaultEntityContainer required on one and only one EntityContainer")
		for p in self.root.FindChildrenDepthFirst(edm.Property):
			try:
				http.MediaType(p.GetAttribute(MimeType))
			except http.HTTPParameterError,e:
				raise InvalidMetadataDocument("MimeType format error in property %s: %s"%(p.name,str(e)))
			except KeyError:
				pass
		...
		for f in self.root.FindChildrenDepthFirst(edm.FunctionImport):
			try:
				if f.GetAttribute(HttpMethod) in (u"POST",u"PUT",u"GET",u"MERGE",u"DELETE"):
					continue
				raise InvalidMetadataDocument("Bad HttpMethod: %s"%f.GetAttribute(HttpMethod))
			except KeyError:
				raise InvalidMetadataDocument("FunctionImport must have HttpMethod defined: %s"%f.name)
		...
		for e in self.root.FindChildrenDepthFirst(edm.CSDLElement):
			try:
				...
				if not isinstance(e,edm.EntityType):
					raise InvalidMetadataDocument("HasStream must only be used on EntityType")
				elif hs not in ("true","false"):
					raise InvalidMetadataDocument("Bad value for HasStream: %s"%hs)				
			except KeyError:
				pass
		...
		try:
			...
			for e in self.root.FindChildrenDepthFirst(edm.CSDLElement):
				try:
					...
					pass
			if version!=match:
				raise InvalidMetadataDocument("Expected version %s, found %s"%(match,version))
			return version
		except KeyError:
			return None
